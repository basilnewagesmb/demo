...
```
index.html            Showcase page: a plain grid of screenshot cards. The site list lives in the SITES array in its <script>.
...
- `SITES` in `index.html` is ordered newest first. Keep the page minimal: screenshot + name only, no extra text.
...
3. Add an entry at the top of `SITES` in `index.html`: `name`, `file`, `shot`.
